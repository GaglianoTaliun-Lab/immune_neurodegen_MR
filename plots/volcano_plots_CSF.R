...
coloc_sign_csf <- list.files(here(project_dir, "results", "cis_MR_CSF", "coloc"), pattern = "significant_PPH4_0.7_*", full.names = TRUE) %>% sort()

coloc_names <- coloc_sign_csf  %>%
  stringr::str_remove(., "/Users/frida/Documents/research-projects/als_immune/results/cis_MR_CSF/coloc/significant_PPH4_0.7_") %>% 
  stringr::str_remove(., "_immune_exposures.tsv") %>% stringr::str_replace_all(., "_", " ") %>%
  stringr::str_replace(., "meta sexcombined", "sex-combined")

coloc_names_out <- coloc_sign_csf  %>%
  stringr::str_remove(., "/Users/frida/Documents/research-projects/als_immune/results/cis_MR_CSF/coloc/significant_PPH4_0.7_") %>% 
  stringr::str_remove(., "_immune_exposures.tsv") %>%
  stringr::str_replace(., "meta_sexcombined", "sex-combined")

loop_names <- coloc_names_out %>% stringr::str_remove_all(., "_CSF") %>% stringr::str_replace(., "sex-combined", "meta_sexcombined")

for (i in 1:length(coloc_sign_csf)) {
  
  coloc <- read.table(stringr::str_c(project_dir, "/results/cis_MR_CSF/coloc/significant_PPH4_0.7_", loop_names[i], "_CSF_immune_exposures.tsv"), sep = "\t", header = T)
  
  MR_results <- read.table(stringr::str_c(project_dir, "/results/cis_MR_CSF/main/MR_main_results_immune_CSF_pqtls_", loop_names[i], ".txt"), sep = "\t", header = T) %>%
    mutate(OR = round(exp(b), 2), 
           ...
           fill_plotord = factor(OR_dir,
                                 levels = c("Decrease", "Increase", "Not significant")))
  
  sign_genes <- read.table(stringr::str_c(project_dir, "/results/cis_MR_CSF/main/MR_significant_main_results_immune_CSF_pqtls_", loop_names[i], ".txt"), sep = "\t", header = T) %>%
    mutate(OR = round(exp(b), 2), 
           OR_dir = case_when(
             OR > 1 & FDR < 0.05 ~ "increase",
             OR < 1 & FDR < 0.05 ~ "decrease",
  ...
  ggplot(MR_results, aes(x = OR, y = -log10(pval))) + 
    geom_point(alpha = 0.8, size = 4, aes(shape = shape_plotord, fill = fill_plotord)) +
    ...
    geom_label_repel(data = sign_genes, aes(label = exposure), fill = "white", alpha= 0.7, force = 2, nudge_y = 0.1, nudge_x = 0.1)
  
  ggsave(here(project_dir, "results", "cis_MR_CSF", "figures", stringr::str_c("volcano_plot_",loop_names[i],"_CSF.jpg")),
