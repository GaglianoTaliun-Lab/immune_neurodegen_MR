# MR PRESSO test for outliers

# 1) load outcome and exposure
# 2) harmonise data
# 3) run MRPRESSO.
...
# colocalized signals:
coloc_sign_results <- try(read.table(here(project_dir, "results", "coloc", "summary_results", stringr::str_c("significant_PPH4_0.7_", pheno_outcome, "_CSF_immune_exposures.tsv")), sep = "\t", header = T), silent = TRUE)
    ...
    for (i in 1:nrow(coloc_sign_results)) { 
        ...
        if (nrow(dat_harmonised_full) > 4) {
            ...
            mrpresso_out <- try(mr_presso(BetaOutcome = "beta.outcome", BetaExposure = "beta.exposure", 
                ...
                NbDistribution = 10000,  SignifThreshold = 0.05), silent = TRUE)
            
            if (!inherits(coloc_sign_results, "try-error")) {
                cat("Writing MR PRESSO results between ", pheno_exposure, " (exposure) and ", pheno_outcome, " (outcome)...\n")
                write.table(mrpresso_out, here(project_dir, "MR", "results", "sensitivity_tests", stringr::str_c("MR_PRESSO_CSF_pqtls_",pheno_exposure,"_",pheno_outcome,".txt")), sep = "\t", row.names = F, quote = F)
                cat("Done.\n")
            } else {
