...
MR_file <- list.files(here(project_dir, "MR", "results", "validation"), pattern = "MR_main_results_immune_ARICvalidation_pqtls_", full.names = TRUE) %>%
    .[array]

pheno_outcome <- MR_file %>%
stringr::str_remove(., "/home/fridald4/projects/def-gsarah/fridald4/als_immune/MR/results/validation/") %>%
    stringr::str_remove(., "MR_main_results_immune_ARICvalidation_pqtls_") %>%
...
exposure_files <- list.files(here(project_dir, "MR", "exposures", "aric_pqtls"), full.names = F, pattern = "clumped_exposure_gene_*")
...
# ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
        ...
        exposure_data <- read.table(here(project_dir, "MR", "exposures", "aric_pqtls", stringr::str_c("clumped_exposure_gene_",pheno_exposure,".txt")), sep = "\t", header = T)
...
# ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
    ...
    rbindlist(dir_results) %>%
        write.table(., here(project_dir, "MR", "results", "validation", stringr::str_c("directionality_test_pqtls_aric_", pheno_outcome, ".txt")), sep = "\t", quote = F)
}