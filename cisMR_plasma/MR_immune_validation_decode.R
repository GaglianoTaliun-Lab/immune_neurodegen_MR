...
for (i in 1:nrow(sign_exposures)) { 
    ...
    exposure_data <- try(read.table(here(project_dir, "MR", "exposures", "decode_pqtls", stringr::str_c("clumped_exposure_gene_", gene, ".txt")), sep = "\t", header = T), silent = TRUE)
...
# ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
    ...
    if (nrow(outcome_data > 0)) {
        ...
        write.table(dat_harmonised, here(project_dir, "MR", "harmonised_data", stringr::str_c("data_harmonised_deCODE_",gene,"_",pheno_outcome,".txt")), sep = "\t", row.names = F, quote = F)
        ...
        if (nrow(mr_ivw_results[[i]]) > 0) {
            if (mr_ivw_results[[i]]$method %in% c("Inverse variance weighted", "Wald ratio") & mr_ivw_results[[i]]$pval < 0.05) {
                ...
                if (nrow(dat_harmonised) == 1) {
                    ...
                    write.table(het, here(project_dir, "MR", "results", "sensitivity_tests", stringr::str_c("heterogeneity_MR_",gene,"_",pheno_outcome,"_decode_pqtls.txt")), sep = "\t", row.names = F, quote = F)
                    cat("Done.\n")
                }
                ...
                if (nrow(dat_harmonised) < 3) {
                    ...
                    write.table(plt, here(project_dir, "MR", "results", "sensitivity_tests", stringr::str_c("pleiotropy_test_",gene,"_",pheno_outcome,"_decode_pqtls.txt")), sep = "\t", row.names = F, quote = F)
                    cat("Done.\n")
                }
                ...
                if (nrow(dat_harmonised) > 1) {
                    mr_leaveoneout(dat_harmonised) %>%
                        write.table(., here(project_dir, "MR", "results", "sensitivity_tests", stringr::str_c("LeaveOneOut_",gene,"_",pheno_outcome,"_decode_pqtls.txt")), sep = "\t", row.names = F, quote = F)
                    cat("Done.\n")
                } else if (nrow(dat_harmonised) == 1) {
                    cat("Skipping since there is only one genetic instrument.\n")
                }
                ...
                if (nrow(dat_harmonised) > 1) {
                    sin <- mr_singlesnp(dat_harmonised) %>%
                        write.table(., here(project_dir, "MR", "results", "sensitivity_tests", stringr::str_c("SingleSNP_",gene,"_",pheno_outcome,"_decode_pqtls.txt")), sep = "\t", row.names = F, quote = F)
                    cat("Done.\n")
                } else if (nrow(dat_harmonised) == 1) {
                    cat("Skipping since there is only one genetic instrument.\n")
                }
...
# ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
                ...
                ggexport(plots, filename=here(project_dir, "MR", "results","validation",stringr::str_c("plots_deCODE_pqtls_",gene,"_",pheno_outcome,".pdf")))    
...
write.table(mr_results_df, here(project_dir, "MR", "results", "validation", stringr::str_c("MR_main_results_immune_deCODEvalidation_pqtls_",pheno_outcome,".txt")), sep = "\t", row.names = F, quote = F)

write.table(mr_results_egger, here(project_dir, "MR", "results", "validation", stringr::str_c("MR_Egger_main_results_immune_deCODEvalidation_pqtls_",pheno_outcome,".txt")), sep = "\t", row.names = F, quote = F)
