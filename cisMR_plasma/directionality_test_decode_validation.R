...
# MR results for array number:
MR_file <- list.files(here(project_dir, "MR", "results", "validation"), pattern = "MR_main_results_immune_deCODEvalidation_pqtls_", full.names = TRUE) %>%
    .[array]

pheno_outcome <- MR_file %>%
    stringr::str_remove(., "/home/fridald4/projects/def-gsarah/fridald4/als_immune/MR/results/validation/") %>%
    stringr::str_remove(., "MR_main_results_immune_deCODEvalidation_pqtls_") %>%
    stringr::str_remove(., ".txt")

mr_sign_results <- read.table(MR_file[1], sep = "\t", header = T) %>%
    filter(., method %in% c("Inverse variance weighted", "Wald ratio")) %>%
    filter(., pval < 0.05)

exposure_files <- list.files(here(project_dir, "MR", "exposures", "decode_pqtls"), full.names = F, pattern = "clumped_exposure_gene_*")
sample_size_exp=34287

N_outcomes <- read.table(here(project_dir, "gwas_sample_sizes_sexstr_neurodegen.tsv"), sep = "\t", header = T)

# ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
# Load outcome and obtain r for directionality test
# ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
...
# ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
        ...
        exposure_data <- read.table(here(project_dir, "MR", "exposures", "decode_pqtls", stringr::str_c("clumped_exposure_gene_",pheno_exposure,".txt")), sep = "\t", header = T)
...
# ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
    ...
    rbindlist(dir_results) %>%
        write.table(., here(project_dir, "MR", "results", "validation", stringr::str_c("directionality_test_pqtls_decode_", pheno_outcome, ".txt")), sep = "\t", quote = F)
}