# Two sample MR between ALS, AD and PD (outcomes, sex-stratified and sex-combined) and immune plasma pQTLs from UKB.
# Exposures were clumped in separate script with 1KGP.
# Downloaded only immune proteins (from ImmPort gene list), so no need to further filter which proteins to keep.
...
for (i in 1:length(exposure_files)) { 
    ...
    gene = exposure_files[i] %>% stringr::str_remove_all(., ".txt") %>% stringr::str_remove_all(., "clumped_exposure_UKB_")
    ...
    if (nrow(exposure_data) < 1){
        cat("The exposure data frame is empty. Skipping to the next.\n")
        next
    }
    ...
    if (nrow(mr_ivw_results[[i]]) > 0) {
        if (mr_ivw_results[[i]]$method %in% c("Inverse variance weighted", "Wald ratio") & mr_ivw_results[[i]]$pval < 0.05) {
            ...
            if (nrow(dat_harmonised) == 1) {
                ...
                write.table(het, here(project_dir, "MR", "results", "sensitivity_tests", stringr::str_c("heterogeneity_MR_UKB_",gene,"_",pheno_outcome,"_pqtls.txt")), sep = "\t", row.names = F, quote = F)
                cat("Done.\n")
            }
            ...
            if (nrow(dat_harmonised) < 3) {
                ...
                write.table(plt, here(project_dir, "MR", "results", "sensitivity_tests", stringr::str_c("pleiotropy_test_UKB_",gene,"_",pheno_outcome,"_pqtls.txt")), sep = "\t", row.names = F, quote = F)
                cat("Done.\n")
            }
            ...
            if (nrow(dat_harmonised) > 1) {
                mr_leaveoneout(dat_harmonised) %>%
                    write.table(., here(project_dir, "MR", "results", "sensitivity_tests", stringr::str_c("LeaveOneOut_UKB_",gene,"_",pheno_outcome,"_pqtls.txt")), sep = "\t", row.names = F, quote = F)
                cat("Done.\n")
            } else if (nrow(dat_harmonised) == 1) {
                cat("Skipping since there is only one genetic instrument.\n")
            }
            ...
            if (nrow(dat_harmonised) > 1) {
                sin <- mr_singlesnp(dat_harmonised) %>%
                    write.table(., here(project_dir, "MR", "results", "sensitivity_tests", stringr::str_c("SingleSNP_UKB_",gene,"_",pheno_outcome,"_pqtls.txt")), sep = "\t", row.names = F, quote = F)
                cat("Done.\n")
            } else if (nrow(dat_harmonised) == 1) {
                cat("Skipping since there is only one genetic instrument.\n")
            }
...
# ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
                ...
                ggexport(plots, filename=here(project_dir, "MR", "results","figures",stringr::str_c("plots_pqtls_UKB_",gene,"_",pheno_outcome,".pdf")))    
...
write.table(mr_results_df, here(project_dir, "MR", "results", "main", stringr::str_c("MR_main_results_immune_pqtls_UKB_",pheno_outcome,".txt")), sep = "\t", row.names = F, quote = F)

write.table(mr_results_egger, here(project_dir, "MR", "results", "main", stringr::str_c("MR_Egger_main_results_immune_pqtls_UKB_",pheno_outcome,".txt")), sep = "\t", row.names = F, quote = F)

filter_fstat %>% mutate(N_instruments_removed = N_before - N_after) %>%
    write.table(., here(project_dir, "MR", "results", "sensitivity_tests", stringr::str_c("Fstatistics_N_instruments_pqtls_UKB_",pheno_outcome,".txt")), sep = "\t", row.names = F, quote = F)
