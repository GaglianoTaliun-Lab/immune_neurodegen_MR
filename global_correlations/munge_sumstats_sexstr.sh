...
study=$(sed -n ${SLURM_ARRAY_TASK_ID}p ${project_dir}/gwas_sample_sizes_sexstr_neurodegen.tsv | awk '{print $1}')
total_N=$(sed -n ${SLURM_ARRAY_TASK_ID}p ${project_dir}/gwas_sample_sizes_sexstr_neurodegen.tsv | awk '{print $4}')

python ${project_dir}/tools/ldsc/munge_sumstats.py \
	--sumstats ${project_dir}/gwas_sumstats/munge_sumstats_tsv/tmp_${study}.tsv \
	--out ${project_dir}/gwas_sumstats/ldsc_sumstats/${study} \
	--N ${total_N} \
	--chunksize 500000 \
